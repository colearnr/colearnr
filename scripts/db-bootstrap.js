...
db.system.js.save({
  ...
  value: function (path, topic) { var args = {id: topic}
    if (path && path != '') { args['path'] = path; } return db.topics.findOne(args, {_id: 1}); }
})

db.system.js.save({
  _id: 'getIdForTopics',
  value: function (topicstr) { var topics = JSON.parse(topicstr)
    var l = []
    topics.forEach(function (atopic) { l.push(getIdForTopic(atopic)); }); return l; }
})
