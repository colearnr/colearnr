...
const WELCOME_TPL = path.resolve(__dirname, '../email/templates/welcome.html')
const INVITE_TPL = path.resolve(__dirname, '../email/templates/invite.html')
const INVITE_COLLAB_TPL = path.resolve(__dirname, '../email/templates/invite_collab.html')
const INVITE_COLEARNR_TPL = path.resolve(__dirname, '../email/templates/invite_colearnr.html')
const PASSWORD_RESET_TPL = path.resolve(__dirname, '../email/templates/password_reset.html')
...
const user = {
  ...
  _name: function (displayName) {
    let name = {}
    if (displayName) {
      let capname = util.capitalise(displayName)
      let tmpA = capname.split(' ')
      let fn = ''
      let mn = ''
      let ln = ''
      fn = tmpA[0]
  ...
  addUserPerms: function (user, topic, cb) {
    ...
    permlib.allowedPerms(user, topic, function (err, perms) {
      ...
      if (perms && perms[permlib.getKey(topic)]) {
        let plist = perms[permlib.getKey(topic)]
  ...
  welcome_user: function (user) {
    let aemail = user.emails.length ? user.emails[0] : ''
    if (aemail) {
      ...
      db.user_invites.findOne({email: aemail}, function (err, invite) {
        ...
        if (!invite) {
          let message = ejs.render(fs.readFileSync(WELCOME_TPL, 'utf8'),
            {
  ...
  invite_user: function (user, invited_by) {
    ...
    db.user_invites.findOne({email: user.email, signed_up: false}, function (err, invite) {
      ...
      if (!invite) {
        let message = ejs.render(fs.readFileSync(INVITE_TPL, 'utf8'),
          {
            filename: INVITE_TPL,
  ...
  invite_collaborator: function (user, invited_by, topic) {
    ...
    let self = this
    let path = (topic.path) ? topic.path : (',' + topic.id + ',')
    db.user_invites.findOne({
      ...
      if (!invite) {
        let message = ejs.render(fs.readFileSync(INVITE_COLLAB_TPL, 'utf8'),
          {
  ...
  invite_colearnr: function (user, invited_by, topic) {
    ...
    db.user_invites.findOne({email: user.email, path: new RegExp('^' + path)}, function (err, invite) {
      ...
      if (!invite) {
        let message = ejs.render(fs.readFileSync(INVITE_COLEARNR_TPL, 'utf8'),
          {
            filename: INVITE_COLLAB_TPL, topic: topic,
            invited_by: invited_by, user: user, base_url: config.base_url + (config.use_port ? (':' + config.port) : '')
          })
  ...
  notify_password_reset: function (user, tmpPassword) {
    let message = ejs.render(fs.readFileSync(PASSWORD_RESET_TPL, 'utf8'),
      {
