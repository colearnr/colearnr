...
function ensureConditionalAccess (req, res, next) {
  ...
  let topicname = req.params['topicname'] || null
  let topic_oid = req.params['topic_oid'] || req.params['topic_id'] || null
  let userId = req.params['userId'] || null
  let contentId = req.params['contentId'] || null
  let tokenId = req.params['tokenId'] || null
  ...
  if (contentId && tokenId) {
    ...
    let domain = util.guessDomain(req.headers['user-agent'])
    let options = {}
    if (domain) {
