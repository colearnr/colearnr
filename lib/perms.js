const Acllib = require('acl')
const MemoryBackend = Acllib.memoryBackend
const MongodbBackend = Acllib.mongodbBackend
...
const perms_db_uri = require('../common/perms_db')
const moment = require('moment')
let acl = new Acllib(new MemoryBackend())

mongodb.connect('mongodb://' + perms_db_uri, function (err, db) {
  if (!err) {
    acl = new Acllib(new MongodbBackend(db, 'acl_', false))
  }
})
...
const perms = {
  ...
  setTestBackend: function () {
    ...
    acl = new Acllib(new MemoryBackend())
  },
  ...
  setTopicRolePerms: function (user, topic, role, callback) {
    let self = this
    acl.removeUserRoles('' + user._id, [constants.COLEARNR_ROLE, constants.ADMIN_ROLE, constants.TOPIC_EXPERT_ROLE + self.getKey(topic), constants.PROLEARNR_ROLE, constants.TOPIC_COLEARNR_ROLE + self.getKey(topic), constants.TOPIC_COLLAB_ROLE + self.getKey(topic), constants.TOPIC_ADMIN_ROLE + self.getKey(topic)], function () {
            ...
            if (err) {
              ...
              acl.areAnyRolesAllowed(roles, '' + self.getKey(topic), perm, function (err, result) {
                ...
                if (!result && (topic.added_by === constants.COLEARNR_ROLE && constants.VIEW_PERMS === perm && topic.draft_mode !== true)) {
                  ...
                  self.addFreeTopicRolePerms(topic, function () {})
  ...
  checkDate: function (user, topic, perm, callback) {
    let isCollab = topic.collaborators && _.indexOf(topic.collaborators, user._id) !== -1
    let isCoLearnr = topic.colearnrs && _.indexOf(topic.colearnrs, user._id) !== -1
    let isOwner = (topic.added_by === user._id)
    let isAdmin = user.roles && _.indexOf(user.roles, constants.ALL_TOPIC_ADMIN_ROLE) !== -1
    ...
    if (topic.startdate || topic.enddate) {
      let now = moment()
      let startdate = topic.startdate ? moment(topic.startdate, 'YYYY-MM-DD HH:mm') : null
      let enddate = topic.enddate ? moment(topic.enddate, 'YYYY-MM-DD HH:mm') : null
      let ret = true
