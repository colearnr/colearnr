var db = require('../../common/db'),
  ...
  convertor = require('../office-convertor'),
...
(function(Optimise) {
  ...
  var out = fs.openSync(pathlib.join(config.cl_log_dir, 'optimise.log'), 'a'),
    err = fs.openSync(pathlib.join(config.cl_log_dir, 'optimise.log'), 'a');
  ...
  Optimise.processLearnbit = function(lbit, callback) {
    var tmpA = lbit.url.split('/'),
      fname = (tmpA && tmpA.length) ? util.purify(tmpA[tmpA.length - 1]) : null,
      urlPrefix = '/lbit/' + lbit._id + '/media/';
    switch (lbit.type) {
      case 'pdf':
  ...
  Optimise.processExternalSourceLearnbit = function(lbit, fname, urlPrefix, callback) {
    ...
    url = util.encode_s3_url(lbit.url);
  ...
  Optimise.processPdfLearnbit = function(lbit, fname, urlPrefix, callback) {
    var added_by = lbit.added_by,
      optimised_url = urlPrefix + fname.replace(/.pdf$/, '_[*,2].pdf'),
      pdf_json_url = urlPrefix + fname.replace(/.pdf$/, '_{page}.js'),
      pdf_png_url = urlPrefix + fname.replace(/.pdf$/, '_{page}.png'),
      thumb_url = urlPrefix + fname.replace(/.pdf$/, '_thumb_{page}.png'),
      img_url_list = [urlPrefix + fname.replace(/.pdf$/, '_thumb_1.png')];
