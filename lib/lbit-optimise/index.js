...
var convertor = require('../office-convertor')
var GridFS = require('../gridfs')
var pathlib = require('path')
var spawn = require('child_process').spawn

;(function (Optimise) {
  var out = fs.openSync(pathlib.join(config.cl_log_dir, 'optimise.log'), 'a')
  var err = fs.openSync(pathlib.join(config.cl_log_dir, 'optimise.log'), 'a')
  var CACHE_DIR = config.cl_cache_dir || '/tmp'
  ...
  var pushArtefactsToGrid = function (lbitId, userId, pdfFile) {
    var artefactDir = pathlib.join(CACHE_DIR, lbitId)
    if (!fs.existsSync(artefactDir)) {
      log.warn('Artefacts directory', artefactDir, 'not found!')
      return
    }
    var files = fs.readdirSync(artefactDir)
    files.filter(function (file) {
      return fs.statSync(pathlib.join(artefactDir, file)).isFile()
    }).forEach(function (file) {
      var fullPath = pathlib.join(artefactDir, file)
  ...
  Optimise.prepare = function (url, fname, lbitId, userId, callback) {
    ...
    if (url.indexOf(constants.CL_PROTOCOL) === 0) {
      GridFS.readFile(lbitId, null, null, function (err, filestream) {
        ...
        var outputFile = pathlib.join(CACHE_DIR, '' + lbitId, fname)
      ...
      var child = spawn('sh', [config.cl_scripts_dir + '/download.sh', url, fname, lbitId, userId, Optimise.CLOUD_PREFIX + userId], {stdio: ['ignore', out, err]})
      ...
      child.on('exit', function (code, signal) {
        var res = { lbitId: lbitId, userId: userId, file: fname }
  ...
  Optimise.doPdf = function (url, pdfFile, lbitId, userId, callback) {
    var _processFn = function () {
      var child = spawn('sh', [config.cl_scripts_dir + '/pdftk.sh', url, pdfFile, lbitId, userId,
        Optimise.CLOUD_PREFIX + userId], { stdio: ['ignore', out, err] })
      child.on('error', function (err) {
        callback(err, null)
      })
      child.on('exit', function (code, signal) {
        var res = { lbitId: lbitId, userId: userId, file: pdfFile }
        if (code === 0) {
    ...
    // Is this cl protocol
    if (url.indexOf(constants.CL_PROTOCOL) === 0) {
      var outputFile = pathlib.join(CACHE_DIR, '' + lbitId, pdfFile)
      if (!fs.existsSync(outputFile)) {
        GridFS.readFile(lbitId, null, null, function (err, filestream) {
          if (err) {
            callback(err, null)
  ...
  Optimise.processLearnbit = function (lbit, callback) {
    ...
    var fname = (tmpA && tmpA.length) ? util.purify(tmpA[tmpA.length - 1]) : null
    var urlPrefix = '/lbit/' + lbit._id + '/media/'
    switch (lbit.type) {
      case 'pdf':
  ...
  Optimise.processOfficeLearnbit = function (lbit, fname, urlPrefix, callback) {
    Optimise.prepare(lbit.url, fname, '' + lbit._id, lbit.added_by, function (err) {
      if (err) {
        ...
        var downloaded_file = pathlib.join(CACHE_DIR, '' + lbit._id, fname.replace(/%20/g, ' '))
        if (fs.existsSync(downloaded_file)) {
          convertor.convert(downloaded_file, 'pdf', function (err) {
            if (err) {
              ...
              var ext = pathlib.extname(fname)
              var pdfFile = fname.replace(ext, '.pdf')
  ...
  Optimise.processExternalSourceLearnbit = function (lbit, fname, urlPrefix, callback) {
    function handleComplete (err, res) {
      var optimised_url = urlPrefix + 'opt-' + lbit._id + '.mp4'
      if (!err) {
    ...
    var userId = lbit.added_by
    if (util.isSupportedVideoSource(lbit.url, lbit.type)) {
      var downloadLocation = '/tmp/' + lbit._id + '/orig-' + lbit._id + '.mp4'
      ...
      video.on('end', function () {
        ...
        var child = spawn('sh', [config.cl_scripts_dir + '/video_optimise.sh', downloadLocation, lbit._id, userId, Optimise.CLOUD_PREFIX + userId], {stdio: ['ignore', out, err]})
        ...
        child.on('exit', function (code, signal) {
          var res = {lbitId: lbit._id, userId: userId, file: fname}
  ...
  Optimise.processPdfLearnbit = function (lbit, fname, urlPrefix, callback) {
    var added_by = lbit.added_by
    var optimised_url = urlPrefix + fname.replace(/.pdf$/, '_[*,2].pdf')
    var pdf_json_url = urlPrefix + fname.replace(/.pdf$/, '_{page}.js')
    var pdf_png_url = urlPrefix + fname.replace(/.pdf$/, '_{page}.png')
    var thumb_url = urlPrefix + fname.replace(/.pdf$/, '_thumb_{page}.png')
    var img_url_list = [urlPrefix + fname.replace(/.pdf$/, '_thumb_1.png')]
