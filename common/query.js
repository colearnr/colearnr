...
var query = {
      ...
      db.topics.findAndModify({
        ...
        function(err, atopicNew) {
          self.get_all_direct_sub_topics(curr_path, atopic.id, true, function(err, stopics) {
            ...
            var oldpath = '^' + (curr_path ? curr_path : ',') + atopic.id + ',';
      ...
      if (atopic) {
        ...
        if (util.empty(oid)) { // Topic is new
          var newTopic = {
            ...
            order: order,
            added_by: added_by,
            privacy_mode: (added_by && 'colearnr' == added_by) ? 'public' : 'private',
            safe: true,
  ...
  create_topic: function(path, name, id, order, author, discuss_id, permission_key, callback) {
    var args = {
      ...
      added_date: new Date(),
      added_by: (author ? author : 'colearnr'),
      safe: true,
      privacy_mode: (author && 'colearnr' == author) ? 'public' : 'private',
      moderation_required: false,
      last_updated: new Date()
    };
  ...
  has_watched: function(user, lbit_oid, topic_oid, duration, callback) {
    ...
    var self = this;
    var MIN_PERCENT = 0.1;
    var duration = (duration) ? parseInt(duration) : null;
    db.vanalytics.find({
