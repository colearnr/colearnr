...
$.fn.editInPlace.defaults = {
	...
	bg_over:			"#486D98", // string: background color of hover of unactivated editor
	...
	save_button:		'<button class="inplace_save btn btn-primary">Save</button>', // string: image button tag to use as “Save” button
	cancel_button:		'<button class="inplace_cancel btn btn-link">Cancel</button>', // string: image button tag to use as “Cancel” button
	...
	default_text:		"", // string: text to show up if the element that has this functionality is empty
	...
	textarea_rows:		5, // integer: set rows attribute of textarea, if field_type is set to textarea. Use CSS if possible though
	...
	on_blur:			null, // string: "save" or null; what to do on blur; will be overridden if show_buttons is true
...
$.extend(InlineEditor.prototype, {
	...
	addHoverEffect: function() {
		if (this.settings.hover_class) {
			...
			this.dom.css("color", this.settings.text_over);
		}
	},
	...
	removeHoverEffect: function() {
		if (this.settings.hover_class) {
			...
			this.dom.css("color", this.settings.text_out);
		}
	},
	...
	replaceContentWithEditor: function() {
		...
		this.dom.html('<form class="inplace_form" style="display: inline; margin: 0; padding: 0;"></form>')
			.find('form')
				...
				.append('<div class="pull-right inplace_save_btn">' + buttons_html + '</div>');
	},

	createEditorElement: function() {
		if (-1 === $.inArray(this.settings.field_type, ['text', 'number', 'textarea', 'select']))
		...
		else if ("number" === this.settings.field_type) {
			editor = $('<input type="number" min="1" max="250" step="1" style="width: 60px; margin-right: 10px;" ' + this.inputNameAndClass()
				+ ' size="' + this.settings.text_size  + '" />');
		}
		...
		$.ajax({
			...
			error: function(request) {
				...
				if (that.settings.error)
					...
					that.reportError("Failed to save value");
			}
		});
	},
