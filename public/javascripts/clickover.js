!function ($) {
  ...
  Clickover.prototype = $.extend({}, $.fn.popover.Constructor.prototype, {
    ...
    , clickery: function (e) {
      ...
      if (e && e.target && 'clickover' == $(e.target).attr('data-dismiss')) {
        this.debugdestroy();
        return;
      }
      // set popover's dim's
      ...
      this[this.isShown() ? 'debughide' : 'debugshow']();
      ...
      if (this.isShown()) {
        ...
        $('body').on(this.attr.click_event_ns, function (e) {                    
          if (e && e.target && ($(e.target).attr('id') == 'lbit_embed' || $(e.target).attr('id') == 'linkEmbedPopup' || $(e.target).hasClass('popover-title')
            || $(e.target).hasClass('popover-content') || $(e.target).hasClass('apollo-seperator'))) {
            // e.preventDefault();
            e.stopPropagation();
            return;
          } else if (that.isShown() && ($(e.target).attr('id') == 'wall' || $(e.target).attr('id') == 'wrapper' || $(e.target).attr('id') == 'structure')) {
            that.debughide();
            return;
          } else {
            that.debughide();
    ...
    , isShown: function () {
      var ret = this.tip().hasClass('in');
      //console.log('isShown', ret);
      return ret;
    }
    ...
    debugdestroy: function () {
      ...
      //console.log(dt + ": clickover destroy");
      this.tip().removeClass('in');
      this.hide();
    }
  })
