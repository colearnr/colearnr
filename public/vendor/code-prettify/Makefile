...
TAR_ROOT=distrib/code-prettify
DIST_JS=dist

all: distrib

clean:
	rm -rf distrib.tstamp distrib src/prettify.js src/run_prettify.js $(DIST_JS)
...
distrib: distrib.tstamp distrib/prettify-small.tgz distrib/prettify-small.zip distrib/prettify-small.tar.bz2 distrib/js
...
distrib/prettify-small.tar: distrib.tstamp
	tar cf $@ -C distrib code-prettify

distrib/prettify-small.zip: distrib.tstamp
	@pushd distrib >& /dev/null; \
	rm -f ../$@; \
	zip -q -9 -r ../$@ code-prettify; \
	popd >& /dev/null

distrib/prettify.tar: distrib.tstamp
	mkdir -p distrib/sources/code-prettify
	cp -fr CHANGES.html COPYING README.html Makefile \
	  examples js-modules src styles tests tools \
	  distrib/sources/code-prettify
	tar cf distrib/prettify.tar -C distrib/sources code-prettify

distrib/js: distrib.tstamp
	mkdir -p $(DIST_JS)
	cp -rf $(TAR_ROOT)/prettify.js $(DIST_JS)/
	cp -rf $(TAR_ROOT)/prettify.css $(DIST_JS)/
